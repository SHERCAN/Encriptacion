...
try:
    ...
    import qrcode
text_to = ['Text to encrypt', 'Text to decrypt',
           'Text encript', 'Text decrypt']
...
def ejecutar():
    ...
    if ch_enc_var.get():
        ...
        img = qrcode.make(token.decode())
        type(img)  # qrcode.image.pil.PilImage
        img.save("encript.png")
    ...
    if ch_dec_var.get():
        ...
        try:
            ...
            messagebox.showinfo(master=root, title='message', message='Error '+str(e))
...
if __name__ == '__main__':
    ...
    ch_enc = tk.Checkbutton(master=frame, text='Encrypt',
                            command=state1, variable=ch_enc_var)
    ch_enc.grid(row=2, column=0, sticky='w')
    ch_dec = tk.Checkbutton(master=frame, text='Decrypt',
                            command=state2, variable=ch_dec_var)
    ...
    boton = tk.Button(master=frame, text='Ejecutar',
                      height=13, command=ejecutar)
